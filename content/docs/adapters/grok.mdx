...
Remote MCP clients (for example xAI) can pair Grok with the hosted Registry MCP at `https://mcp.commercegateway.io/mcp`; protocol details are in [Hosted Overview](/docs/hosted/overview). For HTTP resolution and `RegistryClient`, see [Discovery & Resolution](/docs/registry/discovery).
