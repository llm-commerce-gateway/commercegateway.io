...
To resolve brands, GTINs, or categories to gateway endpoints before tool calls, use the [Commerce Registry Protocol](/docs/registry/overview) (`@betterdata/commerce-registry-protocol`) and the HTTP resolution API, or `createRegistryClient` from `@betterdata/commerce-gateway` — see [Discovery & Resolution](/docs/registry/discovery).
