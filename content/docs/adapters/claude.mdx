...
```ts
import { AnthropicAdapter } from "@commercegateway/commerce-gateway/adapters";
...
- Related: [Boss Loops + Commerce Gateway](/docs/getting-started/concepts#relationship-to-boss-loops)

## Commerce Registry

To resolve brands, GTINs, or categories to gateway endpoints before tool calls, use the [Commerce Registry Protocol](/docs/registry/overview) (`@betterdata/commerce-registry-protocol`) and the HTTP resolution API, or `createRegistryClient` from `@commercegateway/commerce-gateway` — see [.well-known File](/docs/registry/well-known).
